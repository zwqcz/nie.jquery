...
/**
* jQuery lightBox 插件 NIE版
* 此插件根据 Lokesh Dhakar (http://www.huddletogether.com/projects/lightbox2/) 所写的lightbox所创建，使得它可以jQuery的方式使用，并由GOVO修改成中国大陆流行的图片导航方式，并把所有前景图片修改成由CSS控制的背景图。<br>
* <a href="nie.use.html"><font color="red">nie.use模块：</font></a>ui.lightBox<br>
* 原作者插件网站：<a href="http://leandrovieira.com/projects/jquery/lightbox/" target="_blank">http://leandrovieira.com/projects/jquery/lightbox/</a><br>
* 版本: 0.6.nie<br><br>
* <font color="blue"><b>例子：</b></font>点击查看<a href="/demo/ui/lightBox/1.html" target="_blank">demo 1</a><br>
*	<pre>
*	//例子1
*	$('#gallery a').lightBox();
*
...
*			containerBorderSize:10,				//此数值必须与CSS里 #lightbox-container-image-box 的padding左和右值相等，默认为10px
...
*	@param	{Object} settings 参考例子、原作者文档
**/
...
(function($) {
	...
	$.fn.lightBox = function(settings) {		
		...
		settings = jQuery.extend({
			...
			imageArray:				[],
			activeImage:			0,
            controlBar:'bottom' //(string) controlBar position
		},settings);
		...
		function _set_image_to_view() { // show the loading
			...
			$('#lightbox-container-image-data-box').css('opacity',0);
			...
			objImagePreloader.onload = function() {
				...
				var arrPageScroll = ___getPageScroll();
				// get Image Height
                // Modified by govo 20121109: 修正对无缓存图片的高度获取
                var lboxHeight = Math.max($('#lightbox-image').height(),objImagePreloader.height);
				var lbheight = (arrPageSizes[3]-lboxHeight)>0?(arrPageSizes[3]-lboxHeight)/2:0;
                //console.log(arrPageSizes[3],$('#lightbox-image').height(),lbheight,arrPageScroll[1]);
				$('#jquery-lightbox').css({
					top:	arrPageScroll[1] + lbheight,
					left:	arrPageScroll[0]
				});
				// redefined window.resize
				$(window).resize(function() {
					// Get page sizes
					var arrPageSizes = ___getPageSize();
					// Style overlay and show it
		...
		function _show_image_data() {
			...
			$('#lightbox-container-image-data-box').animate({'opacity':1},'slow');
		...
		function _set_navigation() {
			...
			var _action_Navigation_Go=function(mark){
				$('#lightbox-nav-btn'+mark)
						...
						.attr('title',settings['txt'+mark])
						.bind('click',function() {
							(mark=="Prev")?settings.activeImage--:settings.activeImage++;
							_set_image_to_view();
							return false;
						});
				};
			var _action_Navigation_End=function(mark){
				...
				$('#lightbox-nav-btn'+mark)
						...
						.attr('title',settings['txt'+(mark=='Prev'?'First':'Last')])
						.bind('click',function(){
							_finish();
							return false;
						});
