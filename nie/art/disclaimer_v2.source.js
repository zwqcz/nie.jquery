...
“读取文章标题”，推荐你也看看。玄幻的世界！极致的3D！情义的天下！玩游戏我只玩#天下3# ！来自网易的重磅推荐，你怎能错过！ 文章地址
		...
		window.nieBugReporter = window.nieBugReporter || function(){
			...
			var height=469,width=605;
			if(/y3|x3|xdw|zd/.test(nie.config.site)){
				height=width=550;
			}
			
			window.open('http://page-bug-report.webapp.163.com/app/report?site={$site}&title={$title}&refer={$refer}'.replace('{$site}',conf.site).replace('{$title}',encodeURIComponent(conf.title)).replace('{$refer}', conf.refer), 'bugReportWindow',"height="+height+",width="+width+",scrollbars=0,location=no,menubar=no,resizable=1,status=no,toolbar=no");
		}
		var bugReporter = '';
