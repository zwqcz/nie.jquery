		...
		render_and_getForm=function(namesData,formIndex){			
			var _getItemHTML=function(index,nameData){
					...
					if(nameData.name=="captcha") _plusCode='<span class="captcha-wrap hideItem qr-loading"></span>';
						...
						regProductID = regProduct[productName]?regProduct[productName]:productName,
						regUrl = encodeURIComponent(finishPage[productName]?finishPage[productName]:"http://"+productName+".163.com/download/"),
						regPage="http://reg.163.com/reg/reg2.jsp?product="+regProductID+"&url="+regUrl+"&loginurl="+regUrl;							
						_extraCode='<p class="qr-noEmail"><a target="_blank" href="'+regPage+'">还没有邮箱？</a></p>';
						finishPage=regProduct=productName=regProductID=regUrl=regPage=null;
					}
				...
				if(!layer[holderName]){
					layer[holderName]=new function(){
						var _self=this,
							...
							_setClickClose=function(selector){
								return _selfConHolder.find(selector).click(function(){
									_self.hide();
								});
							},
							_selfConHolder=$("<div>",{"class":"NIE-quickReg-msg-con qr-con-"+(holderName=="is163"?"right":holderName)}),//内容容器
						...
						switch(holderName){							
							case "is163":								
								this.show=function(json){									
									_display('<p class="qr-info">注册成功！</p><a href="'+json.url+'" target="_blank" class="btn">确 认</a>');
									_setClickClose("a");
								};								
								break;							
							case "not163":
							...
							case "uploadSMS":
								this.show=function(json){
									_display('<p class="qr-info">编辑短信获取验证码</p><p class="qr-reason">请用手机 <span class="qr-f-hiKey">'
											...
											+'</span> 获取验证<br><span class="qr-f-lowKey">短信费用由运营商收取（0.1元/条）</span></p>'
									,_getCloseBtn("确认","btn"));								
								};		
								break;
							case "reVer":
