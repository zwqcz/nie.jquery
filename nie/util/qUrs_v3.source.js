				...
				if(!layer[holderName]){
					layer[holderName]=new function(){
						...
						switch(holderName){							
							...
							case "reVer":
								this.show=function(json){									
									_display('<p class="reVer-l1">本次注册需要进行短信验证，请按以下提示操作</p>\
											<p class="reVer-l2"><input class="NIE-reVerMobileInp" placeholder="输入手机号码"><button>获取验证码</button><br>\
											<span></span>\
											<input class="NIE-reVerInp" placeholder="输入短信验证码"></p>\
											<p class="reVer-l3">\
											<a href="javascript:void(0);" class="btn">完成注册</a>\
											</p>');
									_selfConHolder.find("button").click(function(){
										var _mobile=$.trim(_selfConHolder.find("input.NIE-reVerMobileInp").val()),
											_span=_selfConHolder.find("span");
										if(/^(13|14|15|18)\d{9}$/.test(_mobile)){
											_span.empty();
											remote("sendVer2MobileCaptcha",{mobile:_mobile},function(json){
												_span.text(json.success?"已发送短信验证码,请查收。":json.msg);
											});
										}
										else _span.text("请正确输入手机号码。");
									});
									_selfConHolder.find("a.btn").click(function(){
										var _mobile=$.trim(_selfConHolder.find("input.NIE-reVerMobileInp").val()),
											_captcha=$.trim(_selfConHolder.find("input.NIE-reVerInp").val());
										if(_mobile!=""&&_captcha!=""){
											var _itemVals=allItems.currentItems.getItemVals();
											_itemVals["antiMobile"]=$.trim(_selfConHolder.find("input.NIE-reVerMobileInp").val());
											_itemVals["captcha"]=$.trim(_selfConHolder.find("input.NIE-reVerInp").val());
											_itemVals["pAnti"]=1;
											submitFn(_itemVals,function(){
												_self.hide();
											});
										}else alert("请完整输入手机号码和短信验证码。");
									});
								};
								break;
							case "sendEMail":
