...
*		 
*		 //	点击注册系统返回信息触发的函数
*		 //	默认为bigUrs.regComplete=this.showMsg;*		 
...
*			 bigself_urs.getVal("username");//获取用户填写的信息。参数为input name值。
		  ...
		  this.getMsgHTML=function(){		  
			  var data = self_urs.regData,
			  	  html ="<div class='not163MailBox'>"
			  			...
			  			+"<li>再次<a href='"+self_urs.aHref+"' target='_self' class='sendMailBtn'>发送注册验证邮件</a>；</li>"
			  			+"<li>如果重发注册验证邮件依然没有收到，请<a class='reReg' target='_self' href='"+self_urs.aHref+"'>重新注册</a>。</li>"
			  			+"</ol></li></ul>"
			  			+"</div>";	
			...
			if(typeof args=="object"){
			  ...
			  if(self_urs.msgO.msg().length==0){
				$("<div>",{
					...
					css:msgCss,
					html:'<div id="'+self_urs.msgID.title+'">注册游戏帐号</div><a id="'+self_urs.msgID.closeBtn+'" href="'+self_urs.aHref+'">X</a><div id="'+self_urs.msgID.con+'"></div>'
				}).appendTo($(document.body));			
				...
				if(regResult && !is163Mail && status==202){
					...
					//需要二次输入验证码
						...
						var submitID="NIE-reVerSubmit",
							btnID="NIE-reVerBtn";
						tmpCon.addClass("reVer").html("<p class='reVer-l1'>只差一步就完成了！<br>请您再输入一次验证码以完成注册：<br><input id='"+self_urs.msgID.reVerInp+"' /><br><span id='NIE-reVerImg'></span></p><p class='reVer-l2'>看不清楚 <a href='"+self_urs.aHref+"' id='"+btnID+"'>换一个</a></p><p class='reVer-l3'><a id='"+submitID+"' href='"+self_urs.aHref+"' class='btn'>完成注册</a></p>");
						this.load_reVerCap();
		  	...
		  	var inpData={username:{},createEmail:{},password:{},repassword:{},promark:{},captcha:{}/*,realname:{},idno:{},phone:{}*/},
				...
				chk163UserName=function(val){
					var info,result=false,
						tmp=(self_urs.type==1)?null:'字母开头，6到18个字符。包括字母、数字、下划线。';
					if (!/^[a-z]/i.test(val)) {
				...
				/*
				 * mustAsy:是否强制执行异步函数。（优先执行于self_urs.progress，暂时用于表达提交必须优先执行）
				 */
				...
				*/			
				...
				self_urs.form.attr("action","#").submit(function(){
					...
					inpData["promark"].pass=(self_urs.promark!="");//判断promark是否符合标准
